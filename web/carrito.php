<?php
  INCLUDE 'config.php';
  $carrito = $pdo -> query ("SELECT prendas.nombre,carrito.id_prenda, carrito.cantidad, prendas.precio FROM carrito
                             inner join prendas on carrito.id_prenda = prendas.id_prenda
...
<div class="container">
  <div class="row">
    <div class="col-15">
      <h1 class="display-2"> Tus productos </h1>
    </div>
    <div class="col">
      <br>
    ...
    <div class="col">
      <div class="container-lg">
        ...
        <thead>
          ...
          <?php
            WHILE ($row =$carrito ->fetch())
                {//<div class="text-center">
                  echo '<tr>';
                    echo '<td>'.$row['nombre'].'</td>';
                    if ($row['id_prenda']=="1") {
                      echo '<td>'.'<div class="text-center"><img src="ropa/1.jpg" width="170" height="170" class="rounded" ></div>'.'</td>';
                    }
                    elseif ($row['id_prenda']=="2") {
                      echo '<td>'.'<div class="text-center"><img src="ropa/2.jpg" width="170" height="170" class="rounded" ></div>'.'</td>';
                    }
                    elseif ($row['id_prenda']=="3") {
                      echo '<td>'.'<div class="text-center"><img src="ropa/3.jpg" width="170" height="170" class="rounded" ></div>'.'</td>';
                    }
                    elseif ($row['id_prenda']=="4") {
                      echo '<td>'.'<div class="text-center"><img src="ropa/4.jpg" width="170" height="170" class="rounded" ></div>'.'</td>';
                    }
                    elseif ($row['id_prenda']=="5") {
                      echo '<td>'.'<div class="text-center"><img src="ropa/5.jpg" width="170" height="170" class="rounded" ></div>'.'</td>';
                    }
                    elseif ($row['id_prenda']=="6") {
                      echo '<td>'.'<div class="text-center"><img src="ropa/6.jpg" width="170" height="170" class="rounded" ></div>'.'</td>';
                    }
                    //echo '<td>'.'<img src="ropa/'+$row['id_prenda']+'.jpg" class="rounded" ></div>'.'</td>';
              ...
              if(isset($_POST['btnE']))
                {
                  if (!empty ($_POST)){
                   ...
                   $sql = "DELETE from carrito where id_factura = (select max(id_factura) from carrito)";
                        $query = $pdo -> prepare($sql);
                        $result = $query -> execute([]);
                  }
                  echo '<p class="alert alert-danger">Se eliminaron del carrito</p>';
                }
              ?>
          </div>
          <div class="col">
            ...
            <input type="submit" value="Eliminar productos" class="btn btn-danger" name="btnE">
            </form>
